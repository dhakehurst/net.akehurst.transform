...
public class AbstractTransformer implements Transformer {
	...
	<S, T> T applyRule(Relation<S, T> r, S source) throws RelationNotFoundException {
		...
		if (target == null) {
			target = r.construct(source, this); // may throw
												// RelationNotFoundException
			this.recordMaping(ruleType, source, target);
			r.configure(source, target, this); // may throw
												// RelationNotFoundException
		}
		return target;
	}
			...
			for (Relation rule : rules) {
				...
				if (exceptionCount == rules.size()) {
					throw new RelationNotFoundException(
							"No relation " + ruleType + " found that is appicable to " + source);
				}
			}
		}
		return null;
	...
	public <S, T> List<? extends T> transformAll(Class<? extends Relation> ruleType, List<? extends S> element)
